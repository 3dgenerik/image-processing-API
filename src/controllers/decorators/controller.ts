...
export const controller = (routePrefix: string) => {
  return (target: Function) => {
    ...
    for (const key of targetPrototypeNames) {
      ...
      const middleware =
        Reflect.getMetadata(AppFeatures.MIDDLEWARE, target.prototype, key) ||
        [];
      const validation =
        Reflect.getMetadata(AppFeatures.VALIDATOR, target.prototype, key) || [];
      if (path && method) {
        router[method](
          `${routePrefix}${path}`,
          [...middleware],
          queryParamsValidation(validation),
